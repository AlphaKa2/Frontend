...
import { faUpload, faMoon, faSun, faLock } from "@fortawesome/free-solid-svg-icons";
import GetUserProfileApi from "../../api/blog-services/profile/GetUserProfile";
import UpdateUserProfileApi from "../../api/blog-services/profile/UpdateUserImageApi";
  ...
  fileInput.onchange = async (event) => {
    ...
    if (file) {
      try {
        ...
        const { status, data } = await RequestS3urlApi({
          fileName: file.name,
        ...
        if (status === 200) {
          // S3에 이미지 업로드
          await RequestImageUpdateApi(s3Url, file);
...
// 회원탈퇴
  ...
  const handleConfirmCancelAccount = async () => {
    try {
        const response = await CancelAccountApi();
        if (response == 200) {
  ...
  const handleSave = async() => {
    ...
    try {
    const response = await UpdateUserProfileApi(userId, nickname, profileDescription);
    console.log("Response in handleSave:", response.data); // 추가된 디버깅 로그
  ...
  <div className="flex justify-between items-center w-full mb-4">
    <div className="text-gray-700 font-semibold text-lg">프로필 사진</div>
  ...
  >
    <FontAwesomeIcon icon={faUpload} className="text-blue-500 mb-2" />
    <p className="text-sm text-gray-400">
      여기에 이미지를 드래그 앤 드롭하거나<br />
      SVG, PNG, JPG 혹은 GIF (최대 800x800px) 업로드 버튼을 클릭하세요.
    </p>
    <input
      type="file"
      accept="image/*"
            ...
            <div>
              ...
              <textarea
                ...
                onChange={ (e) => {
                  ...
                  if (e.target.value.trim()) {
                    setErrors((prev) => ({ ...prev, profileDescription: false }));
                  }}
                }
                rows="6"
